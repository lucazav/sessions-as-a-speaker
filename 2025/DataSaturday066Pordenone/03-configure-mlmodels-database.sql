...
IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'MLModels')
BEGIN
	CREATE DATABASE [MLModels];
END
ELSE
BEGIN
	PRINT 'Database MLModels already exists.';
END
GO
...
IF NOT EXISTS (SELECT * FROM sys.external_languages WHERE [language] = 'py310')
BEGIN
...
-- Let's try to call an sp_execute_external_script within a user-defined function
IF OBJECT_ID (N'dbo.fn_test_call_external_script', N'FN') IS NOT NULL
    DROP FUNCTION dbo.fn_test_call_external_script;
GO
CREATE FUNCTION dbo.fn_test_call_external_script(@test_id int)
RETURNS int
AS
BEGIN
    DECLARE @ret int = @test_id;
...
print(sys.path)
print(sys.version)
print(sys.executable)';EXEC sp_execute_external_script
@language =N'py310',
@script=N'
import sys
print(sys.path)
...
SELECT id = dbo.fn_test_call_external_script(1);
...
IF OBJECT_ID('dbo.stp_generate_binary_model', 'P') IS NOT NULL
DROP PROCEDURE dbo.stp_generate_binary_model;
GO
...
IF OBJECT_ID('dbo.stp_predict_titanic_survivors', 'P') IS NOT NULL
DROP PROCEDURE dbo.stp_predict_titanic_survivors;
GO
...
BEGIN
	...
	-- Build the input query to get all the input values for the model
	...
	-- Select the model to use for scoring by name and version
	SELECT TOP 1
		@binary_model = model
	FROM dbo.models
	WHERE
	...
	-- Get the output score for the input values using the model
	EXECUTE sp_execute_external_script @language = N'py310'
			, @script = N'
import pandas as pd
import pickle
